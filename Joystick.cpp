...
HRESULT UpdateInputState( HWND hDlg )
{
	...
	int i = 0;
	int nDataLength;
	string baseip = "255.255.255.255";

		string joyveryables = "X:"+ std::to_string(js.lX)+ "," + "Y:" + std::to_string(js.lY) + "," + "Z:"+ std::to_string(js.lZ) + ",";
		...
		// send GET / HTTP
		send(Socket, joyveryables.c_str(), strlen(joyveryables.c_str()), 0);
		...
		// SetWindowText(GetDlgItem(hDlg, IDC_NetResponse), wstmp.c_str());
